...
The eviction threshold must sit below the claim gate — otherwise the runner could idle itself permanently with no cleanup ever triggering; startup warns and clamps if it isn't. Engaging the claim gate kicks a cleanup immediately (debounced per episode) rather than waiting for the periodic sweep.
